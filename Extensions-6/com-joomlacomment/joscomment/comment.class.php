...
class JOSC_visual extends JOSC_properties {
	...
	{
	    return '<div id="poweredby" align="center" class="small">Powered by <a target="_blank" href="http://compojoom.com">!JoomlaComment '."4.0 beta1".'</a></div>';
	}

	function insertHiddenCopyright()
	{
        return '<h4 style="display:none;">!joomlacomment 4.0 Copyright (C) 2009 Compojoom.com . All rights reserved."</h4>';
	}
    ...
    function getComments($onlydata=false)
    {
        ...
        if ($this->_display_num>0) {
        	...
        	$this->_total = $database->loadResult();
                $this->_number_of_comments = $this->_total;
		...
		$database->SetQuery($querycount.$queryfrom.$queryparent.$queryorder);
        	$this->_number_of_comments = $database->loadResult();
...
class JOSC_menu extends JOSC_support {
    ...
    function menu_htmlCode()
    {
        ...
        if ($display) {
            ...
            $html 	= str_replace('{BUTTON_RSS_URL}', JRoute::_("index.php?option=com_comment&no_html=1&josctask=rss&contentid=$this->_content_id"), $html);
        }
        return $html;
    }
}
...
//		$debug = true;
		...
		if (($nblines=count($TxtTags))<=1) {
			$parsedText = JOSC_utils::wrapWord($parsedText, $maxlength_word);
...
//				$is_tag = false;
			...
			$is_tag = ($tagpos=JString::strpos($line, $TO))===false ? false : true;

			if ($is_tag) {
				if ($debug)	$text .= "<br />DEBUG_istag".$line."<br />";
				/* push tags , pop end tags */
				$tagkey	= JString::substr($line, 0, $tagpos);  			 /* xxx<... */
				if ($tagkey) {
					$line	= JString::substr_replace($line, '', 0, $tagpos);  /* xxx<... */
					if (JString::substr($line,1,1)=='/') {
						if (!(($pos=JString::strpos($push, $tagkey))===false))
               	    		/* pop */
               	    		$push = JString::substr_replace($push,'',$pos-2,JString::strlen($tagkey)+3);
					} else {
						/* push */
				...
				if (!(JString::strpos($line, $limittextTag)===false)) {
...
class JOSC_form extends JOSC_support {
	...
	function form_htmlCode() {
	    ...
	    if(in_array($id[0], $disabled_more_comments)) {
        	/* needed informations but hidden : */
