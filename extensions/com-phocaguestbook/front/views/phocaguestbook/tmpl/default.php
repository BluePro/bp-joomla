...
function submitbutton() {
	...
	if ($this->tmpl['display_title_form']== 2) {?>
		...
		alert("<?php echo JText::_( 'Phoca Guestbook No Subject', true); ?>");return false;} <?php }
...
if ($this->tmpl['display_posts'] == 1) {
	...
	foreach ($this->items as $key => $values) {
		...
		if (function_exists('str_ireplace')) {
			$values->content	= str_ireplace ('../plugins/editors/tinymce3/', 'plugins/editors/tinymce3/', $values->content);
			$values->content	= str_ireplace ('../plugins/editors/tinymce/', 'plugins/editors/tinymce/', $values->content);
		} else {		
			$values->content	= str_replace ('../plugins/editors/tinymce3/', 'plugins/editors/tinymce3/', $values->content);
...
if ($this->tmpl['show_form'] == 1) {
	...
	if ((int)$this->tmpl['display_name_form'] > 0){
		echo '<tr>'
		.'<td><strong>'.JText::_('Name').PhocaguestbookHelper::getRequiredSign((int)$this->tmpl['display_name_form']).' </strong></td>'
		.'<td colspan="3">'
	...
	if ((int)$this->tmpl['display_email_form'] > 0){
		echo '<tr>'
		.'<td><strong>'.JText::_('Email').PhocaguestbookHelper::getRequiredSign((int)$this->tmpl['display_email_form']).' </strong></td>'
		.'<td colspan="3">'
	...
	if ((int)$this->tmpl['display_website_form'] > 0){
		echo '<tr>'
		.'<td><strong>'.JText::_('Website').PhocaguestbookHelper::getRequiredSign((int)$this->tmpl['display_website_form']).' </strong></td>'
		.'<td colspan="3">'
	...
	if ((int)$this->tmpl['display_content_form'] > 0){
		echo '<tr>'
		.'<td><strong>'.JText::_('Content').PhocaguestbookHelper::getRequiredSign((int)$this->tmpl['display_content_form']).' </strong></td>'
