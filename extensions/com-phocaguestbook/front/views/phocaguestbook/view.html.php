...
class PhocaGuestbookViewPhocaGuestbook extends JView
{
	function display($tpl = null) {
		...
		PhocaGuestbookHelper::checkSpecificId(0);
		global $mainframe;
		...
		//PARAMS
		$tmpl['in']						= PhocaguestbookHelper::getPGBI();
		...
		$tmpl['display_posts']			= $params->get( 'display_posts', 1 );
		$tmpl['recaptcha_publickey']	= $params->get( 'recaptcha_publickey', '' );
		...
		if ((int)$tmpl['enable_editor'] > 0) {
			$document->addCustomTag(PhocaguestbookHelper::setTinyMCEJS($tmpl['enable_editor']));
		...
		*/
		...
		$tmpl['captcha_id']				= PhocaguestbookHelper::getCaptchaId($tmpl['enable_captcha']);
		...
		if (JRequest::getVar( 'title-msg-1', 0, 'get', 'int' ) == 1) {
			$tmpl['errmsg_top'] .= $smB . JText::_( 'Phoca Guestbook No Subject' ).$smE;
		}
		if (JRequest::getVar( 'title-msg-2', 0, 'get', 'int' ) == 1) {
			$tmpl['errmsg_top'] .= $smB . JText::_( 'Phoca Guestbook Bad Subject' ). $smE;
		}
